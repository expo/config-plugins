...
function setDebugInfoPlistBuildSetting(value: string | undefined) {
  ...
  if (value === undefined) {
    delete objects.XCBuildConfiguration[debugConfigurationId].buildSettings
      .INFOPLIST_FILE;
  } else {
    objects.XCBuildConfiguration[
      debugConfigurationId
    ].buildSettings.INFOPLIST_FILE = value;
  }
...
function addMisleadingExtensionInfoPlist() {
  ...
  } of configurationList.buildConfigurations) {
...
function manifestReferencesDelegate(
  value: unknown,
  className: string,
): boolean {
  ...
  if (value !== null && typeof value === "object") {
    return Object.values(value).some((item) =>
      manifestReferencesDelegate(item, className),
    );
  }
  ...
  function visitGroup(groupId: string, parentPath: string) {
    ...
    for (const { value: childId } of group.children) {
      if (objects.PBXGroup[childId]) {
        ...
        const basePath =
          unquote(file.sourceTree) === "SOURCE_ROOT"
...
function seedUserOwnedState() {
  ...
  fs.writeFileSync(
    "/app/ios/HelloWorld/UserFeature.swift",
    "struct UserFeature {}\n",
  );
...
function snapshotOwnedState() {
  const generated = fs.existsSync(generatedDirectory)
    ? Object.fromEntries(
        readGeneratedSwiftSources().map(({ file, contents }) => [
          file,
          contents,
        ]),
      )
...
describe(withExpoUIScene, () => {
  ...
  it.each([
        ...
        UIWindowSceneSessionRoleApplication: [
          {
            ...
            UISceneDelegateClassName: expect.stringMatching(
    ...
    for (const { file } of sources) {
      expect(
        applicationSources.filter(
          ({ absolutePath }) =>
            absolutePath === path.join(generatedDirectory, file),
        ),
      ).toHaveLength(1);
    }
    ...
    const factoryClass = appDelegate.match(
      /let factory = (\w+)\(delegate: delegate\)/,
    )?.[1];
    ...
    expect(generatedSource).toContain(
      `class ${factoryClass}: RCTReactNativeFactory`,
    );
    expect(generatedSource).toContain(
    ...
    expect(generatedSource).toMatch(
      /performActionFor:\s*shortcutItem,[\s\S]*completionHandler:/,
    );
  ...
  it("rejects an unmarked collision at the generated source path", async () => {
    ...
    fs.writeFileSync(
      path.join(generatedDirectory, "ExpoUIScene.swift"),
      "// User-owned source\n",
    );

    await expect(runPlugin()).rejects.toThrow(
      /expo-uiscene.*generated source.*already exists/i,
    );
    expect(
      fs.readFileSync(
        path.join(generatedDirectory, "ExpoUIScene.swift"),
  ...
  it("removes only plugin-owned integration when disabled", async () => {
    ...
    seedUserOwnedState();
    const originalAppDelegate = fs.readFileSync(appDelegatePath, "utf8");
    const originalInfoPlist = plist.parse(
    ...
    expect(
      fs.readFileSync("/app/ios/HelloWorld/UserFeature.swift", "utf8"),
    ).toBe("struct UserFeature {}\n");
  ...
  it("removes plugin-owned integration when manifest key order changes", async () => {
    ...
    expect(fs.existsSync(generatedDirectory)).toBe(false);
    expect(
      plist.parse(fs.readFileSync(infoPlistPath, "utf8"))
        .UIApplicationSceneManifest,
    ).toBeUndefined();
  });
  ...
  it("does not remove generated support before a later Info.plist modifier finishes", async () => {
    ...
    expect(
      manifestReferencesDelegate(manifest, "ExpoUISceneSceneDelegate"),
    ).toBe(true);
    expect(
    ...
    expect(fs.existsSync(generatedDirectory)).toBe(true);
    expect(fs.readFileSync(appDelegatePath, "utf8")).toContain(
      "ExpoUISceneReactNativeFactory",
    );
  ...
  it("validates the application target Info.plist when an extension plist is the shorter glob match", async () => {
    ...
    expect(
      plist.parse(fs.readFileSync("/app/ios/X/Info.plist", "utf8"))
        .UIApplicationSceneManifest,
    ).toBeUndefined();
  });
  ...
  it("atomically rejects disable when Debug uses a distinct plist that retains the generated delegate", async () => {
    ...
    expect(
      manifestReferencesDelegate(
        plist.parse(fs.readFileSync(debugInfoPlistPath, "utf8"))
          .UIApplicationSceneManifest,
        "ExpoUISceneSceneDelegate",
      ),
    ).toBe(true);
  ...
  it.each([
    ...
    "atomically rejects disable when Debug %s",
    async (_label, buildSetting, expectedError) => {
      loadProject();
      await runPlugin({ enabled: true });
  ...
  it.each([
    ...
    "atomically rejects disable when the final manifest uses %s",
    async (delegateReference) => {
      loadProject();
      await runPlugin({ enabled: true });
      ...
      (contents: string) =>
        contents
          .replace(
            ...
            "factory.startReactNativeCustom(",
          )
          .concat("\n// factory.startReactNative(\n"),
    ],
  ])(
    "rejects a template missing the standard %s structure",
    async (_label, mutate) => {
      loadProject((files) => {
